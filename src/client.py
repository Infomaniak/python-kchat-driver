...
and actually makes the requests to the kchat server
"""
...
log = logging.getLogger("kchatdriver.websocket")
...
class Client:
    def __init__(self, options):
        self._url = "{scheme:s}://{url:s}:{port:d}{basepath:s}".format(
            scheme=options["scheme"], url=options["url"], port=options["port"], basepath=options["basepath"]
        )
